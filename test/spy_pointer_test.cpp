  ...
  : public enable_spying
{
  ...
  virtual void foo() {}
  virtual void on_spying_state_changed() { is_spied_state_in_callback = is_spied(); }
};
...
{
  SpiedObject obj;
  EXPECT_FALSE(obj.is_spied_state_in_callback);
  spy_pointer<SpiedObject> spy(&obj);
  EXPECT_TRUE(obj.is_spied_state_in_callback);
  spy.reset();
  EXPECT_FALSE(obj.is_spied_state_in_callback);
}
...
TEST(spying, the_onli_first_and_last_spy_changes_spying_state)
{
  ...
  obj.is_spied_state_in_callback = false;
  spy_pointer<SpiedObject> spy2(&obj);
  EXPECT_FALSE(obj.is_spied_state_in_callback);

  obj.is_spied_state_in_callback = true;
  spy2.reset();
  EXPECT_TRUE(obj.is_spied_state_in_callback);
}
